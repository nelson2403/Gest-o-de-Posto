...
interface TaxaRow {
  taxa_debito: number | null; taxa_credito: number | null; taxa_credito_parcelado: number | null
  posto: string; adquirente: string; empresa: string
}
...
function DreTab() {
  ...
  const [postosSel,  setPostosSel]  = useState<string[]>([]) // ids de postos selecionados
  ...
  useEffect(() => {
    supabase.from('postos').select('id, nome, codigo_empresa_externo').eq('ativo', true).order('nome')
      .then(({ data }) => {
        if (data) setPostos(data as PostoMap[])
      })
  }, [])
  ...
  // Agrupa por posto para tabela DRE
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Erro */}
      {erro && (
        <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-[13px] text-red-700">
          <strong>Erro ao buscar dados:</strong> {erro}
        </div>
      )}
      ...
      {!loading && dados.length === 0 && !erro && (
        <div className="text-center py-16 text-gray-400 text-[13px]">
          ...
          Selecione o período e clique em <strong>Buscar</strong> para carregar o DRE.
        </div>
      )}
                      ...
                      <tbody>
                        {p.rows.map((r, i) => (
                          <tr key={i} className={cn('border-b border-gray-50 hover:bg-gray-50/60', i % 2 === 0 && 'bg-white')}>
                            ...
                            <td className="py-2 px-3 text-right font-mono text-gray-500">
                              {r.taxa_esperada != null ? fmtPct(r.taxa_esperada) : '—'}
                            </td>
                            <td className="py-2 px-3 text-right">
                              <DreDivergenciaBadge real={r.taxa_efetiva} esperada={r.taxa_esperada} />
                            </td>
                        ...
                        <tr className="bg-gray-100 font-semibold border-t border-gray-200">
                          ...
                          <td className="py-2 px-3 text-right font-mono text-purple-800">
                            {p.bruto > 0 ? fmtPct(p.taxas / p.bruto * 100) : '—'}
                          </td>
                          <td colSpan={2} />
...
export default function AnaliticoPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      const [resMaq, resTax] = await Promise.all([
        supabase.from('maquininhas').select('status, valor_aluguel, posto:postos(nome, empresa:empresas(nome)), adquirente:adquirentes(nome)'),
        supabase.from('taxas').select('taxa_debito, taxa_credito, taxa_credito_parcelado, posto:postos(nome, empresa:empresas(nome)), adquirente:adquirentes(nome)'),
      ])
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const buildEmpresa = (r: any) => r?.posto?.empresa?.nome ?? '—'
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const buildPosto   = (r: any) => r?.posto?.nome ?? '—'
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const buildAdq     = (r: any) => r?.adquirente?.nome ?? '—'

      setMaqRows((resMaq.data ?? []).map(r => ({ status: r.status, valor_aluguel: r.valor_aluguel ?? null, posto: buildPosto(r), adquirente: buildAdq(r), empresa: buildEmpresa(r) })))
      setTaxRows((resTax.data ?? []).map(r => ({ taxa_debito: r.taxa_debito ?? null, taxa_credito: r.taxa_credito ?? null, taxa_credito_parcelado: r.taxa_credito_parcelado ?? null, posto: buildPosto(r), adquirente: buildAdq(r), empresa: buildEmpresa(r) })))
      setLoading(false)
    }
    load()
  }, [])
  ...
  const barAlugAdq   = useMemo(() => { const map: Record<string, number> = {}; comAlug.forEach(r => { map[r.adquirente] = (map[r.adquirente] ?? 0) + (r.valor_aluguel ?? 0) }); return Object.entries(map).sort((a, b) => b[1] - a[1]).map(([adq, total]) => ({ adq, total })) }, [comAlug])
  const barAlugPosto = useMemo(() => { const map: Record<string, number> = {}; comAlug.forEach(r => { map[r.posto] = (map[r.posto] ?? 0) + (r.valor_aluguel ?? 0) }); return Object.entries(map).sort((a, b) => b[1] - a[1]).slice(0, 12).map(([posto, total]) => ({ posto, total })) }, [comAlug])

  const barTaxaAdq = useMemo(() => {
    const map: Record<string, { deb: number[]; cred: number[]; parc: number[] }> = {}
    taxRows.forEach(r => {
      if (!map[r.adquirente]) map[r.adquirente] = { deb: [], cred: [], parc: [] }
      if (r.taxa_debito !== null)            map[r.adquirente].deb.push(r.taxa_debito)
      if (r.taxa_credito !== null)           map[r.adquirente].cred.push(r.taxa_credito)
      if (r.taxa_credito_parcelado !== null) map[r.adquirente].parc.push(r.taxa_credito_parcelado)
    })
    const avg = (arr: number[]) => arr.length ? arr.reduce((s, v) => s + v, 0) / arr.length : 0
    return Object.entries(map).map(([adq, v]) => ({ adq, 'Débito': +avg(v.deb).toFixed(2), 'Crédito': +avg(v.cred).toFixed(2), 'Parcelado': +avg(v.parc).toFixed(2) }))
  }, [taxRows])

  const mediaDeb  = taxRows.filter(r => r.taxa_debito !== null).reduce((s, r) => s + (r.taxa_debito ?? 0), 0) / (taxRows.filter(r => r.taxa_debito !== null).length || 1)
      ...
      <div className="p-6 space-y-6">
            ...
            {loading ? <div className="skeleton h-32 rounded-xl w-full" /> : (
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  ...
                  <KpiCard title="Ativas"      value={String(ativas)}  icon={TrendingUp}  iconColor="text-emerald-600" iconBg="bg-emerald-100" sub={total ? `${((ativas/total)*100).toFixed(1)}%` : undefined} />
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                      <BarChart data={barAdqStatus} margin={{ left: -10, right: 10, top: 4, bottom: 20 }}>
                        ...
                        {Object.entries(STATUS_LABEL).map(([key, label]) => (
                          <Bar key={key} dataKey={key} name={label} stackId="a" fill={STATUS_COLOR[key]} radius={key === 'extraviada' ? [4,4,0,0] : undefined} />
                        ))}
                      </BarChart>
                    </ResponsiveContainer>
                  </ChartCard>
                ...
                <ChartCard title={`Top ${barPostoTop.length} Postos por Quantidade`}>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={barPostoTop} layout="vertical" margin={{ left: 0, right: 30, top: 4, bottom: 4 }}>
                      ...
                      <Tooltip content={<CustomTooltip />} />
                      <Bar dataKey="total" name="Maquininhas" fill="#f97316" radius={[0,4,4,0]} label={{ position: 'right', fontSize: 11, fill: '#94a3b8' }} />
                    </BarChart>
                  </ResponsiveContainer>
                </ChartCard>
              </>
            ...
            {comAlug.length === 0 ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <KpiCard title="Total Mensal" value={fmtBRL(totalMensal)} icon={DollarSign} iconColor="text-emerald-600" iconBg="bg-emerald-100" sub={`${(totalMensal * 12).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} / ano`} />
                  <KpiCard title="Maquininhas c/ Aluguel" value={String(comAlug.length)} icon={Smartphone} iconColor="text-blue-600" iconBg="bg-blue-100" sub={`${((comAlug.length/total)*100).toFixed(1)}% do total`} />
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                      <PieChart>
                        <Pie data={barAlugAdq} cx="50%" cy="50%" innerRadius={65} outerRadius={105} paddingAngle={3} dataKey="total" nameKey="adq"
                          label={({ name, percent }: { name?: string; percent?: number }) => `${(name ?? '').split(' ')[0]} ${((percent ?? 0) * 100).toFixed(0)}%`} labelLine={false}>
                          {barAlugAdq.map((_, i) => <Cell key={i} fill={BAR_COLORS[i % BAR_COLORS.length]} />)}
                        </Pie>
                        <Tooltip content={<CustomTooltip formatter={fmtBRL} />} />
                      </PieChart>
        ...
        {tabAtiva === 'taxas' && (
          <div className="space-y-6">
            ...
            {taxRows.length === 0 ? (
              <Card className="border-gray-200"><CardContent className="p-8 text-center text-gray-400">Nenhuma taxa cadastrada.</CardContent></Card>
            ) : (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <KpiCard title="Média Débito"    value={fmtPct2(mediaDeb)}  icon={Percent} iconColor="text-blue-600"   iconBg="bg-blue-100" />
                  <KpiCard title="Média Crédito"   value={fmtPct2(mediaCred)} icon={Percent} iconColor="text-purple-600" iconBg="bg-purple-100" />
                  <KpiCard title="Média Parcelado" value={fmtPct2(mediaParc)} icon={Percent} iconColor="text-pink-600"   iconBg="bg-pink-100" />
                </div>
                <ChartCard title="Comparativo de Taxas por Adquirente (média %)">
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={barTaxaAdq} margin={{ left: -10, right: 20, top: 4, bottom: 20 }}>
                      <CartesianGrid {...gridProps} />
                      <XAxis dataKey="adq" tick={tickStyle} angle={-15} textAnchor="end" interval={0} />
                      <YAxis tick={tickStyle} tickFormatter={v => `${v}%`} />
                      <Tooltip content={<CustomTooltip formatter={fmtPct2} />} />
                      <Legend wrapperStyle={{ fontSize: 11 }} />
                      <Bar dataKey="Débito"    fill="#3b82f6" radius={[3,3,0,0]} />
                      <Bar dataKey="Crédito"   fill="#8b5cf6" radius={[3,3,0,0]} />
                      <Bar dataKey="Parcelado" fill="#ec4899" radius={[3,3,0,0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </ChartCard>
                <ChartCard title="Tabela de Médias por Adquirente">
                  <div className="overflow-x-auto">
                    <table className="w-full text-[12px]">
                      <thead>
                        <tr className="border-b border-gray-100">
                          <th className="text-left py-2 px-3 font-semibold text-gray-500">Adquirente</th>
                          <th className="text-right py-2 px-3 font-semibold text-blue-600">Débito</th>
                          <th className="text-right py-2 px-3 font-semibold text-purple-600">Crédito</th>
                          <th className="text-right py-2 px-3 font-semibold text-pink-600">Parcelado</th>
                        </tr>
                      </thead>
                      <tbody>
                        {barTaxaAdq.sort((a, b) => a.adq.localeCompare(b.adq)).map((row, i) => (
                          <tr key={i} className={cn('border-b border-gray-50', i % 2 === 0 && 'bg-gray-50/50')}>
                            <td className="py-2.5 px-3 font-medium text-gray-700">{row.adq}</td>
                            <td className="py-2.5 px-3 text-right font-mono text-blue-700">{fmtPct2(row['Débito'])}</td>
                            <td className="py-2.5 px-3 text-right font-mono text-purple-700">{fmtPct2(row['Crédito'])}</td>
                            <td className="py-2.5 px-3 text-right font-mono text-pink-700">{fmtPct2(row['Parcelado'])}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
