...
export default function ComissionamentoMembrosPage() {
  ...
  const [pessoaSelecionada, setPessoaSelecionada] = useState<PessoaAutosystem | null>(null)
  ...
  async function confirmarAdicionar() {
    if (!pessoaSelecionada || !postoAddId) return
    setSalvandoAdd(true)
    try {
      const r = await fetch('/api/comissionamento/membros', {
        ...
        body: JSON.stringify({
          posto_id:           postoAddId,
          external_person_id: String(pessoaSelecionada.grid),
      ...
      const json = await r.json()
      if (!r.ok || json.error) {
        toast({ variant: 'destructive', title: 'Erro ao adicionar', description: json.error })
        return
      }
      toast({
        title: 'Membro adicionado',
        description: `${pessoaSelecionada.nome} cadastrado(a) como ${ROLE_LABELS[roleAdd]}`,
      })
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1">
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between text-[11px]">
                <span className="text-gray-500">
                  {carregandoAS
                    ? 'Carregando...'
                    : `${pessoasDisponiveis.length} funcionário${pessoasDisponiveis.length === 1 ? '' : 's'} disponível${pessoasDisponiveis.length === 1 ? '' : 'is'}`}
                </span>
                {pessoaSelecionada && (
                  <Badge variant="outline" className="text-[10px] gap-1 bg-orange-50 text-orange-700 border-orange-200">
                    <Check className="w-2.5 h-2.5" />
                    {pessoaSelecionada.nome.split(' ').slice(0, 2).join(' ')}
                  </Badge>
                )}
              </div>
              {erroAS && (
              ...
              <div className="max-h-64 overflow-y-auto divide-y divide-gray-100">
                {carregandoAS ? (
                  ...
                  pessoasDisponiveis.slice(0, 50).map(p => {
                    const selecionada = pessoaSelecionada?.grid === p.grid
                    return (
                      <button
                        key={p.grid}
          ...
          <DialogFooter className="pt-4 border-t border-gray-200 mt-2">
            ...
            <Button
              ...
              disabled={!pessoaSelecionada || !postoAddId || salvandoAdd}
