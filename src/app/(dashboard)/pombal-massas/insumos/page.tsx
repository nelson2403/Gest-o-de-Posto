...
import { Plus, Pencil, Trash2, X, Wheat, RefreshCw } from 'lucide-react'
...
const UNIDADES = ['kg', 'g', 'un', 'L', 'ml', 'pct', 'cx']
...
export default function InsumosPage() {
  ...
  const carregar = useCallback(async () => {
    setLoading(true)
    const r = await fetch('/api/pombal-massas/insumos')
  ...
  async function salvar() {
    ...
    try {
      ...
      const url = editando ? `/api/pombal-massas/insumos/${modal.id}` : '/api/pombal-massas/insumos'
      ...
      toast({ title: editando ? 'Matéria-prima atualizada' : 'Matéria-prima criada' })
      setModal(null)
      carregar()
    } finally { setSalvando(false) }
  }

  async function excluir(s: Insumo) {
    if (!confirm(`Excluir "${s.nome}"?`)) return
    const r = await fetch(`/api/pombal-massas/insumos/${s.id}`, { method: 'DELETE' })
    if (!r.ok) { const j = await r.json(); toast({ variant: 'destructive', title: j.error ?? 'Erro' }); return }
    toast({ title: 'Matéria-prima excluída' })
    carregar()
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <Header
        title="Matérias-primas — POMBAL MASSAS"
        description="Insumos usados na produção dos salgados (custo e estoque)"
        actions={
          <button onClick={() => setModal({ unidade: 'kg' })}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800 text-[11px] uppercase tracking-wide text-gray-500">
                  <th className="text-left px-4 py-3">Matéria-prima</th>
                  <th className="text-center px-4 py-3">Unidade</th>
                  <th className="text-right px-4 py-3">Custo unitário</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {lista.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <td className="px-4 py-3 text-right text-gray-600">{fmtBRL(s.custo_unitario)}</td>
                    <td className="px-4 py-3 text-right tabular-nums">{Number(s.estoque).toLocaleString('pt-BR')} {s.unidade}</td>
                    <td className="px-4 py-3 text-right font-semibold text-gray-800 dark:text-gray-200">{fmtBRL(s.estoque * s.custo_unitario)}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="font-semibold text-gray-900 dark:text-gray-100">{modal.id ? 'Editar Matéria-prima' : 'Nova Matéria-prima'}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select value={modal.unidade ?? 'kg'} onChange={e => setModal(m => ({ ...m, unidade: e.target.value }))}
                    className="w-full border border-gray-200 dark:border-gray-700 dark:bg-gray-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
                    {UNIDADES.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Custo unit. (R$)</label>
                  <input type="number" step="0.0001" value={modal.custo_unitario ?? ''} onChange={e => setModal(m => ({ ...m, custo_unitario: parseFloat(e.target.value) }))}
                    className="w-full border border-gray-200 dark:border-gray-700 dark:bg-gray-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Estoque</label>
